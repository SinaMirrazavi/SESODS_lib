...
for pointer_of_K=1:K
    ...
    for pointer_of_colum_and_row=1:2*d
        C(counter)=penalty*((-1)^(pointer_of_colum_and_row+1))*Comstrain_on_Ak(transpose(A)*P(:,:,pointer_of_K)+P(:,:,pointer_of_K)*A,pointer_of_colum_and_row)+penalty*epsilon;
        counter=counter+1;
    end
end 
for pointer_of_K=1:K
    for pointer_of_colum_and_row=1:2*d
        C(counter)=-penalty*Comstrain_on_Ak(P(:,:,pointer_of_K),pointer_of_colum_and_row)+penalty*epsilon;
        counter=counter+1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Derivatives
...
Grad_C_P=zeros(d*K,d*(2*d+1)*K);
 Grad_C_handle=zeros(1,d*(2*d+1)*K);
 counter_main=1;
for pointer_of_K=1:K  
    for pointer_of_colum_and_row=1:2*d
        counter=1; 
        for j=1:K 
             for colum=1:2*d
                 for row=colum:2*d
                    Grad_C_handle(:,counter)=((-1)^(pointer_of_colum_and_row+1))*Diff_constrains_ak_respect_to_P(Sigma_P,Sigma_V,P,d,row,colum,j,pointer_of_colum_and_row,pointer_of_K);
                    counter=counter+1;
                 end 
             end 
        end
...
Grad_C_P_P=zeros(d*K,d*(2*d+1)*K);
 Grad_C_handle=zeros(1,d*(2*d+1)*K);
 counter_main=1;
for pointer_of_K=1:K  
    for pointer_of_colum_and_row=1:2*d
        ...
        for j=1:K 
             for colum=1:2*d
                 for row=colum:2*d
                    Grad_C_handle(:,counter)=-Diff_constrains_P_respect_to_P(P,d,row,colum,j,pointer_of_colum_and_row,pointer_of_K);
                    counter=counter+1;
                 end 
             end 
        end
  ...
  Grad_C=transpose([Grad_C_Position Grad_C_Velocity Grad_C_P;zeros(2*K*d,(K+d*K+d*(3*d+1)*K/2)) zeros(2*K*d,(d*K+d*(3*d+1)*K/2)) Grad_C_P_P]); 
...
output=0;
    if Pointer_of_KK==Pointer_of_K
        A=[zeros(d,d) eye(d,d);Sigma_P(d+1:2*d,1:d,Pointer_of_K)/(Sigma_P(1:d,1:d,Pointer_of_K)) Sigma_V(d+1:2*d,1:d,Pointer_of_K)/(Sigma_V(1:d,1:d,Pointer_of_K))];
        handle=transpose(A)*P(:,:,Pointer_of_K)+P(:,:,Pointer_of_K)*A;
        ...
        handle3=transpose(A)*P(:,:,Pointer_of_K)+P(:,:,Pointer_of_K)*A;
