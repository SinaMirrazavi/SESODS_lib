...
for pointer_of_K=1:K
    ...
    for pointer_of_colum_and_row=1:2*d
        C(counter)=((-1)^(pointer_of_colum_and_row+1))*Comstrain_on_Ak(transpose(A)*P(:,:,pointer_of_K)+P(:,:,pointer_of_K)*A,pointer_of_colum_and_row)+epsilon;
        counter=counter+1;
    end 
end  
for pointer_of_K=1:K
    for pointer_of_colum_and_row=1:2*d
        C(counter)=-Comstrain_on_Ak(P(:,:,pointer_of_K),pointer_of_colum_and_row)+epsilon;
        counter=counter+1;
    end
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Derivatives
